...
目前交代的任务基本上完成的差不多了  除了一些接口的调试以及数据处理，组件的问题之外

因为入门不久  所以很多内容也是参照同事的代码似懂非懂的进行开发，中间也没有任何试错成本

所以踩坑还是比较多的

首先总结一下与小程序开发的主要逻辑不同之处(页面渲染以及js)

开发中最明显的差别就是data数据的改变
小程序均需要setData,而vue中直接修改数据状态即是，好坏暂不了解。

然后即是小程序的bingdchange  以及bingtap 区别，实际上的使用差别并不能体现出啦
甚至不仔细研读API觉得这个设计并不怎么样，对于vue中各种状态的使用，均直接
动态绑定状态:title :style :calss等

关于生命周期函数以及对于数据请求的渲染层面来说，理解尚还不是很深刻

其他的差别在于微信小程序直接是一种原生状态的环境，甚至可以嵌套webview
而我们开发的vue仍然是面向浏览器的，所以仍然存在一些dom操作(当然不好),而小程序基本 靠事件或者自定义data-进行数据获取.


#### mask/遮罩层 的内容相关

Mask即是一个动态切换的选项卡，点击阴影取消显示，点击选项进行响应操作

之前做mask的逻辑都是选项卡做一个，然后mask单独做一个，分别用fixed定位进行定位拼接  在面向移动端时  px 和 rem都不适用的当下就显得尴尬

现在的思路是 整个区域都用Mask包裹，然后透明度为.5的0,0,0 然后在里面渲染子元素，通过子元素不同的背景颜色，以及margin padding 等属性拓宽，不给高度即可

甚至不需要z index属性既可以完成效果
